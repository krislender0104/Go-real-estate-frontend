...
export class PropertyItemComponent implements OnInit {
  ...
  public addToFavorites(){
    ...
    if (currentUser) {
      ...
      // var temp = data.find(this.property.id);
      flag = 2;
      if(data == null)
        data = [];
      if(!data.includes(this.property.id))
      {
        data.push(this.property.id);
        localStorage.setItem("favorites", JSON.stringify(data));
      }
      
    }
    this.appService.addToFavorites(this.property, (this.settings.rtl) ? 'rtl':'ltr', flag);
  }
